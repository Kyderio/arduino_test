...
*/
#define USBD_EP_DESCRIPTION_SIZE(ep_count)			(USB_LEN_CFG_DESC + USB_LEN_IF_DESC + ep_count * USB_LEN_EP_DESC)
#define USBD_EP_COUNT_MAX					(USB_MAX_ENDPOINTS * 2)
/* Exported types ------------------------------------------------------------*/
typedef struct {
	int(* init)(void);
	int(* deinit)(void);
...
typedef struct {
	...
	usbd_ep_t            ep_list[USBD_EP_COUNT_MAX];
	usb_dev_t            *dev;
	usbd_verify_cb_t     *cb;
	u8                   num_endpoints;
	u8                   ctrl_req;
...
/* Exported functions --------------------------------------------------------*/
int usbd_verify_init(const usbd_verify_cb_t *cb);
