...
typedef MediaExtractor *(*CreateFunction)(DataSourceBase *source, void *meta);
typedef void (*ReleaseMetaFunction)(void *meta);

typedef struct {
	void *buffer; // probe buffer
	size_t capacity; //probe buffer capacity
	size_t range_length; //probe buffer available data length
	size_t start_offset; //probe buffer start offset according to audio source
	size_t read_offset; //probe buffer read offset according to audio source
	size_t id3_offset; //id3 offset according to audio source
} ProbeBuffer;
...
typedef CreateFunction(*DetectorFunction)(
	DataSourceBase *source,
	float *confidence, void **meta,
	ReleaseMetaFunction *release_meta);

// Fast detect function.
typedef CreateFunction(*FastDetectorFunction)(
...
typedef struct {
	...
	// the fast Detector function
	FastDetectorFunction fast_detector_function;
} ExtractorType;
