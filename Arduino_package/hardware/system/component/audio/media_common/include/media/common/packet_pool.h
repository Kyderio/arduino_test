...
public:
	/**
	 * The max duration a player can buffer for streaming source.
	 * Not recommended to set it too big, because setting it too large can lead to insufficient memory.
	 * But kMaxBufferDurationForStreaming must be larger than {@link kStreamingBufferingInitialMs} and {@link kStreamingBufferingResumeMs}.
	 * The unit is millisecond.
	 */
	static size_t kMaxBufferDurationForStreaming;

	explicit PacketPool(const sptr<MetaData> &meta, size_t capacity = kMaxNumBuffers);
	...
	virtual res_t ReadMultiple(
		std::vector<RBuffer *> * /* buffers */, uint32_t /* max_num_buffers = 1 */,
	...
	bool HasFreeSpace(bool is_streaming = false);
	...
	int64_t GetBufferedDurationNs(res_t *final_result);

	void QueueDataChunk(const sptr<RBuffer> &buffer);
	...
	res_t DequeueDataChunk(sptr<RBuffer> *buffer);
	void RequeueDataChunk(sptr<RBuffer> &buffer);
...
private:
	...
	{
	public:
		int64_t max_deque_time_ns_, max_enque_time_ns_;
		DiscontinuitySegment()
			: max_deque_time_ns_(-1),
			  max_enque_time_ns_(-1)
		{
		};

		void Clear()
		{
			max_deque_time_ns_ = max_enque_time_ns_ = -1;
		}
	};
	...
	int64_t last_queued_time_ns_;
	std::list<sptr<RBuffer>> buffers_;
	res_t eos_result_;
	sptr<Message> latest_enqueued_meta_;
	sptr<Message> latest_dequeued_meta_;

	size_t buffering_size_;
