...
public:
		  ...
		  play_response_parsed_(false)
	{
		...
		if (user.Size() > 0) {
			...
			MEDIA_LOGE("rewritten session url: '%s'", session_url_.Str());
		}
...
case_1:
	...
	bool PokeAHole(int rtp_socket, int rtcp_socket, const RString &transport)
	{
		...
		if (!GetAttribute(transport.Str(),
						  "source",
						  &source)) {
			MEDIA_LOGW("Missing 'source' field in Transport response. Using "
				   "RTSP endpoint address.");

			struct hostent *ent = gethostbyname(session_host_.Str());
			if (ent == NULL) {
				MEDIA_LOGE("Failed to look up address of session host '%s'",
					   session_host_.Str());
		...
		if (!GetAttribute(transport.Str(),
						  "server_port",
						  &server_port)) {
			MEDIA_LOGI("Missing 'server_port' field in Transport response.");
			return false;
		}
		...
		if (sscanf(server_port.Str(), "%d-%d", &rtp_port, &rtcp_port) != 2
			...
			MEDIA_LOGE("Server picked invalid RTP/RTCP port pair %s,"
		...
		if (rtp_port & 1) {
			MEDIA_LOGW("Server picked an odd RTP port, it should've picked an "
		...
		if (n < (ssize_t)buf->Size()) {
			MEDIA_LOGE("failed to poke a hole for RTP packets");
			return false;
		}
		...
		if (n < (ssize_t)buf->Size()) {
			MEDIA_LOGE("failed to poke a hole for RTCP packets");
			return false;
		}

		MEDIA_LOGV("successfully poked holes.");
	...
	static bool IsLiveStream(const sptr<SessionDescription> &desc)
	{
		...
		if (desc->FindAttribute(0, "a=LiveStream", &attr_live_stream)) {
			...
			if (!desc->GetDurationUs(&duration_us)) {
				MEDIA_LOGV("No duration found, assume live stream");
				return true;
			}
		}
	...
	virtual void OnMessage(const sptr<Message> &msg)
	{
		...
		case kMsgConnected: {
			...
			MEDIA_LOGI("connection request completed with result %d (%s)",
		...
		case 'desc': {
			...
			MEDIA_LOGI("DESCRIBE completed with result %d (%s)",
				   result, strerror(-result));
			...
			if (result == OK) {
				...
				if (response->status_code_ == 301 || response->status_code_ == 302) {
					...
					if (it == response->headers_.end()) {
						...
						MEDIA_LOGE("can not get the location url info");
					} else {
					...
					if (RTSPConnection::ParseURL(
						...
						MEDIA_LOGI("rewritten session url: '%s'", session_url_.Str());
					}
				...
				if (response->status_code_ != 200) {
					...
					MEDIA_LOGE("The response has no content.");
				} else {
					...
					if (!session_description_->IsValid()) {
						MEDIA_LOGE("Failed to parse session description.");
						result = ERROR_MALFORMED;
					} else {
#if 1
						...
						if (!base_url_.StartsWith("rtsp://")) {
							...
							MEDIA_LOGW("Server specified a non-absolute base URL"
						...
						if (session_description_->GetTracksNum() < 2) {
							...
							MEDIA_LOGW("Session doesn't contain any playable "
								   "tracks. Aborting.");
		...
		case 'sdpl': {
			...
			MEDIA_LOGI("SDP connection request completed with result %d (%s)",
				   result, strerror(-result));
		...
		case 'setu': {
			...
			MEDIA_LOGI("SETUP(%d) completed with result %d (%s)",
			...
			if (result == OK) {
				...
				if (response->status_code_ != 200) {
					...
					if (it == response->headers_.end()) {
						session_id_ = "" ;
						MEDIA_LOGE("can not get the session info");
					} else {
					...
					if (GetAttribute(
						...
						if (end == timeout_string.Str() || *end != '\0') {
							MEDIA_LOGW("server specified malformed timeout '%s'",
								   timeout_string.Str());

							keep_alive_timeout_us_ = kDefaultKeepAliveTimeoutUs;
						} else if (timeout_s < 15) {
							MEDIA_LOGW("server specified too short a timeout "
							...
							MEDIA_LOGI("server specified timeout of %lu secs.",
								   timeout_s);
						}
					}
			...
			if (result == OK && index < session_description_->GetTracksNum()) {
				...
				MEDIA_LOGD("[debug1] request msg is= '%s'", request.Str());
		...
		case 'play': {
			...
			MEDIA_LOGI("PLAY completed with result %d (%s)",
				   result, strerror(-result));
		...
		case 'opts': {
			...
			MEDIA_LOGI("OPTIONS completed with result %d (%s)",
				   result, strerror(-result));
		...
		case 'tear': {
			...
			MEDIA_LOGI("TEARDOWN completed with result %d (%s)",
				   result, strerror(-result));
...
#if 1
				MEDIA_LOGI("stream ended? aborting.");
				(new Message('abor', this))->Post();
				break;
#else
				MEDIA_LOGI("haven't seen an AU in a looong time.");
#endif
			}
		...
		case kMsgDataChunk: {
			...
			if (track_index >= tracks_.size()) {
				MEDIA_LOGV("late packets ignored.");
				break;
			}
			...
			if (msg->FindInt32("eos", &eos)) {
				MEDIA_LOGI("received BYE on track index %d", track_index);
				if (!all_tracks_have_time_ && DataReceivedOnAllChannels()) {
					MEDIA_LOGI("No time established => fake existing data");
			...
			if (rewind_pending_) {
				MEDIA_LOGV("we're rewinding, dropping stale packet.");
				break;
			}
		...
		case 'paus': {
			...
			if (count != pause_count_) {
				MEDIA_LOGV("Ignoring outdated pause message.");
				break;
			}

			if (!rewindable_) {
				MEDIA_LOGW("This is a live stream, ignoring pause request.[rewindable_=%d]", rewindable_);
				break;
			}

			if (pausing_) {
				MEDIA_LOGV("This stream is already paused.");
				break;
			}
		...
		case 'pau2': {
			...
			MEDIA_LOGI("PAUSE completed with result %d (%s)",
				   result, strerror(-result));
			break;
		}
		...
		case 'resu': {
			...
			MEDIA_LOGW("[debug3] request msg is= '%s'", request.Str());
		...
		case 'res2': {
			...
			MEDIA_LOGI("PLAY (for resume) completed with result %d (%s)",
				   result, strerror(-result));
			...
			if (result != OK) {
				MEDIA_LOGE("resume failed, aborting.");
				(new Message('abor', this))->Post();
			}
		...
		case 'rwd': {
			if (!rewindable_) {
				MEDIA_LOGW("This is a live stream, ignoring rewind request.");
		...
		case 'see1': {
			...
			MEDIA_LOGW("[debug2] request msg is= '%s'", request.Str());
		...
		case 'see2': {
			...
			MEDIA_LOGI("PLAY (for rewind) completed with result %d (%s)",
				   result, strerror(-result));
			...
			if (result == OK) {
				...
				if (response->status_code_ != 200) {
					...
					if (it == response->headers_.end()) {
						MEDIA_LOGE("can not get the rtp-info ");
					} else {
						MEDIA_LOGV("rtp-info: %s", it->second.Str());
					}
#else
					ssize_t i = response->headers_.indexOfKey("rtp-info");
					CHECK_GE(i, 0);
					MEDIA_LOGV("rtp-info: %s", response->headers_.valueAt(i).Str());
#endif
					MEDIA_LOGI("rewind completed.");
				}
			}

			if (result != OK) {
				MEDIA_LOGE("rewind failed, aborting.");
				(new Message('abor', this))->Post();
			}
		...
		case 'biny': {
			...
			MEDIA_LOGD("[yiyuan]index=%d", index);
		...
		case 'tiou': {
			...
			if (!received_first_rtcp_packet_) {
				if (DataReceivedOnAllChannels() && !try_fake_rtcp_) {
					MEDIA_LOGW("We received RTP packets but no RTCP packets, "
					...
					MEDIA_LOGW("Never received any data, switching transports.");
					...
					MEDIA_LOGW("Never received any data, disconnecting.");
					(new Message('abor', this))->Post();
				}
			} else {
				if (!all_tracks_have_time_) {
					MEDIA_LOGW("We received some RTCP packets, but time "
	...
	{
		MEDIA_LOGV("cancelDataChunkTimeoutCheck");
		++check_count_;
	}
	...
	static void SplitString(
		const RString &s, const char *separator, std::list<RString> *items)
	{
		items->clear();
		size_t start = 0;
		while (start < s.Size()) {
	...
	void ParsePlayResponse(const sptr<RTSPResponse> &response)
	{
		...
		if (tracks_.size() == 0) {
			MEDIA_LOGV("ParsePlayResponse: late packets ignored.");
			return;
		}
		...
		if (!SessionDescription::ParseNTPRange(val.Str(), &npt1, &npt2)) {
			...
			MEDIA_LOGI("This is a live stream");
			return;
		}
		...
		std::list<RString> stream_infos;
		SplitString(rtp_info, ",", &stream_infos);

		int n = 1;
		for (std::list<RString>::iterator it = stream_infos.begin();
			 it != stream_infos.end(); ++it) {
			(*it).Trim();
			MEDIA_LOGV("streamInfo[%d] = %s", n, (*it).Str());
			...
			MEDIA_LOGV("track #%d: rtp_time=%u <=> npt=%.2f, first_data_chunk_=%d", n, rtp_time, npt1, first_data_chunk_);
...
private:
	struct TrackInfo {
		...
		std::list<sptr<RBuffer> > packets_;
	};
	...
	void SetupTrack(size_t index)
	{
		...
		if (source->Check() != OK) {
			MEDIA_LOGW("Unsupported format. Ignoring track #%d.", index);
		...
		MEDIA_LOGE("track #%d URL=%s", tracks_.size(), track_url.Str());
	...
	{
		MEDIA_LOGV("onTimeUpdate track %d, rtp_time = 0x%08x, ntp_time = %#016llx",
		...
		if (!all_tracks_have_time_) {
			...
			if (all_tracks_have_time) {
				...
				MEDIA_LOGI("Time now established for all tracks.");
			}
		}
		if (all_tracks_have_time_ && DataReceivedOnAllChannels()) {
	...
	res_t ProcessDataChunkQueue(int32_t track_index)
	{
		...
		while (!track->packets_.empty()) {
				...
				if (rewindable_) {
					if (track->allowed_stale_data_chunk_num_ > 0) {
						...
						if (seq_num16 > first_seq_num_in_segment16 + kMaxAllowedStaleDataChunks
							...
							MEDIA_LOGV("discarding stale access unit (0x%x : 0x%x)",
								   seq_num, track->first_seq_num_in_segment_);
							continue;
						}
						MEDIA_LOGW_IF(seq_num16 != first_seq_num_in_segment16,
						...
						num_data_chunks_received_ = 0;
						MEDIA_LOGW_IF(track->allowed_stale_data_chunk_num_ == 0,
			...
			if (seq_num < track->first_seq_num_in_segment_) {
				MEDIA_LOGV("dropping stale access-unit (%d < %d)",
					   seq_num, track->first_seq_num_in_segment_);
				continue;
			}
		...
		int32_t track_index, const sptr<RBuffer> &data_block)
	{
		...
		uint32_t seq_num = (uint32_t)data_block->Int32Data();
		MEDIA_LOGV("OnDataChunkComplete track %d storing accessunit %u", track_index, seq_num);

		if (!play_response_parsed_) {
			MEDIA_LOGV("play response is not parsed");
			return;
		}
		...
		if (!all_tracks_have_time_) {
			MEDIA_LOGV("storing data_block, no time established yet");
			return;
		}
		...
		const sptr<RBuffer> &data_block)
	{
		...
		if (media_time_us < 0) {
			MEDIA_LOGV("dropping early data_block.");
			return false;
		}

		MEDIA_LOGV("track %d rtp_time=%u media_time_us = %lld us (%.2f secs)",
