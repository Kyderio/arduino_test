...
#ifndef AMEBA_FWK_MEDIA_PLAYBACK_CORE_MEDIA_CORE_PLAYER_CORE_RENDERER_LITE_H
#define AMEBA_FWK_MEDIA_PLAYBACK_CORE_MEDIA_CORE_PLAYER_CORE_RENDERER_LITE_H
...
class PlayerCoreLite::Renderer : public MessageHandler
{
public:
	Renderer(const sptr<AudioRenderSinkLite> &sink, const sptr<Message> &notify);

	void QueueBuffer(const sptr<RBuffer> &buffer, const sptr<Message> &notify_consumed);
	...
	bool DrainAudio(const sptr<RBuffer> &buffer);
...
private:
	...
	sptr<AudioRenderSinkLite> audio_render_sink_;
	sptr<Message> notify_;
	Mutex lock_;
	//std::list<BufferChunck> audio_chuncks_;
	...
	int64_t audio_anchor_time_media_ns_;
	int64_t audio_time_media_ns_;

	bool notify_complete_audio_;

	// modified on only renderer's thread.
	bool paused_;
	int64_t pause_drain_audio_allowed_ns_; // time when we can drain/deliver audio in pause mode.
	...
	res_t OnOpenAudioRenderSinkLite(
		const sptr<Message> &format,
		bool is_streaming);
	void OnCloseAudioRenderSinkLite();
	...
	//void FlushChuncks(std::list<BufferChunck> *queue);
...
#endif  // AMEBA_FWK_MEDIA_PLAYBACK_CORE_MEDIA_CORE_PLAYER_CORE_RENDERER_LITE_H