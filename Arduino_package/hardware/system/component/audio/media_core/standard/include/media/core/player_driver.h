...
{
public:
    explicit PlayerDriver(RTPlayer *rtplayer, AudioSinkTypes type);
    virtual ~PlayerDriver();

    res_t SetSource(const char *url);
    res_t SetSource(RTDataSource *source);
    res_t Prepare();
    res_t PrepareAsync();
    res_t Start();
    res_t SetLooping(int8_t loop);
    res_t Stop();
    res_t Pause();
    bool IsPlaying();
    ...
    void NotifyDuration(int64_t duration_ns);
...
private:
    sptr<AudioRenderSink> GetAudioOutput(AudioSinkTypes type);
