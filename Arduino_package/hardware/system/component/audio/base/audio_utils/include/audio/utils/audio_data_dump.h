...
{
	MEDIA_LOGD("[Dump]CreateAudioDataInfo channels: %d, rate: %d, path: %s, d_count: %d", channels, rate, file_path, count);
	struct AudioDataInfo *audio_data_info = (struct AudioDataInfo *)osal_malloc(sizeof(struct AudioDataInfo));
	memset(audio_data_info, 0, sizeof(struct AudioDataInfo));

	if (!audio_data_info) {
		MEDIA_LOGE("[Dump]calloc AudioDataInfo failed!");
		return NULL;
	}

	audio_data_info->file = fopen(file_path, "wb");
	if (!audio_data_info->file) {
		MEDIA_LOGE("[Dump]Unable to create file '%s'\n", file_path);
		osal_free(audio_data_info);
		return NULL;
	}
...
{
	//MEDIA_LOGD("[Dump]DumpAudioData bytes: %zu", bytes);
	if (audio_data_info == NULL) {
		MEDIA_LOGE("file error, can not capture audio data");
		return 0;
	}
	...
	if (fwrite(buffer, 1, bytes, audio_data_info->file) != bytes) {
		MEDIA_LOGE("[Dump]Error writing audio data");
		return 0;
	}
...
{
	MEDIA_LOGD("[Dump]DumpAudioDataWavHeader");
	if (!audio_data_info || !audio_data_info->file) {
		MEDIA_LOGE("[Dump]file error, can not write wav header");
		return;
	}
	MEDIA_LOGV("total frames:%d, block align:%d", audio_data_info->frames, audio_data_info->header.block_align);
