...
{
	MEDIA_LOG_ALWAYS_FATAL("MixVolume<int32_t, int32_t, float> Should not be here");
	return raw_data * volume;
}
...
{
	MEDIA_LOG_ALWAYS_FATAL("MixVolume<int16_t, int16_t, float> Runtime Should not be here");
...
inline void MixEffect(TYPE_OUTPUT *raw_effect_data, TYPE_INPUT raw_data)
{
	if (!IS_SAME_TYPE<TYPE_OUTPUT, TYPE_INPUT>::value) {
		MEDIA_LOG_ALWAYS_FATAL("MixEffect type not right: %u %u\n", sizeof(TYPE_OUTPUT), sizeof(TYPE_INPUT));
	}
	...
	if (effect_data != NULL) {
		do {
			...
			case TYPE_MIX_STEREO_ONLY_SAVE:
				...
				break;
			default:
				MEDIA_LOG_ALWAYS_FATAL("invalid mixtype %d", TYPE_MIX);
				break;
			}
