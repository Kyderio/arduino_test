...
protected:
	...
	inline size_t GetInFramesRequsted(size_t out_frame_count)
	{
		return (static_cast<uint64_t>(out_frame_count) * in_sample_rate_
				+ (sample_rate_ - 1)) / sample_rate_;
	}

	inline float RecalVolFloat(float volume)
	{
...
private:
	...
	static bool IsQSupported(src_quality quality);
	static bool EntryInit(void *param);
	static uint32_t QMHz(src_quality quality);
};
