...
{
public:
    AudioStreamRender(AudioHwCard *card);
    virtual ~AudioStreamRender();

    virtual int32_t        CreateRender(
                            const struct AudioHwPathDescriptor *desc,
                            const struct AudioHwConfig *attr);
            void           DestroyRender();
            int32_t        GetRenderPosition(uint32_t *frames);
    virtual int32_t        GetRenderPosition(uint64_t *frames);
    virtual int32_t        GetPresentationPosition(uint64_t *frames, struct timespec *timestamp);
    virtual int32_t        GetTimestamp(RTAudioTimestamp *timestamp);
    virtual int32_t        Write(const void *buffer, size_t bytes);
    virtual size_t         GetFrameSize() const { return hal_frame_size_; }
    virtual uint32_t       GetFormat() const;
    virtual uint32_t       GetSampleRate() const;
    virtual size_t         GetBufferSize() const;
    virtual uint32_t       GetChannels() const;
    virtual int32_t        SetParameters(const char *params) const;
    virtual int32_t        Flush();
    virtual int32_t        Standby();
    uint64_t               FramesWritten() { return frames_written_; }

private:
    AudioHwCard           *HalCard() const;

public:
    AudioHwCard          *audio_card_;
    AudioHwStreamOut     *audio_stream_out_;

protected:
    uint64_t              frames_written_;
    uint64_t              frames_written_at_standby_;
    uint64_t              render_position_;
    int32_t               rate_multiplier_;
