...
template<typename T> class StateSQueue
{
...
public:
	...
	public:
		...
		int32_t Insert(const T &value)
		{
			...
			sequence++;
			osal_atomic_acquire_store(sequence, &shared->sequence_);
			//ameba_atomic_acquire_store(sequence, &shared->sequence_);
			shared->value_ = value;
			sequence++;
			osal_atomic_release_store(sequence, &shared->sequence_);
			//ameba_atomic_release_store(sequence, &shared->sequence_);
			sequence_ = sequence;
			return sequence;
		}

		enum SSQ_STATUS Confirm() const
		{
			const uint32_t ack = osal_atomic_acquire_load(&shared_->ack_);
		...
		bool Confirm(int32_t sequence) const
		{
	...
	public:
		...
		bool GetVal(T &value)
		{
			...
			for (int tries = 0; ;) {
				...
				if (before & 1) {
					...
					osal_atomic_memory_barrier();
					T temp = shared->value_;
					//int32_t after = ameba_atomic_release_load(&shared->sequence_);
					uint32_t after = osal_atomic_release_load(&shared->sequence_);
					if (after == before) {
						value = temp;
						shared->ack_ = before;
						sequence_ = before;
		...
		void done()
		{
			...
			osal_atomic_release_store(ack, &shared_->ack_);
...
#if 0
	StateSQueue(void /*Shared*/ *shared);
	/*virtual*/ ~StateSQueue() { }
