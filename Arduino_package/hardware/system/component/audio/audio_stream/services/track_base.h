...
class ServerThreadBase::ServerTrackBase: public ExtendedServerBufferAgent, public RefPtr
{
...
public:
	...
	ServerTrackBase(ServerThreadBase *thread, uint32_t sample_rate,
					uint32_t format, uint32_t channel_count,
					size_t frame_count, bool is_out);

	virtual                ~ServerTrackBase();

	virtual     int32_t       GetNextAvailBuffer(TrackServerBufferAgent::Buffer *buffer, int64_t pts) = 0;
	...
	void        TrackShareMemInit();   //jackie add

	const wptr<ServerThreadBase> track_in_thread_;
	share_mem_block       *track_cblk_;
	...
	ServerAgent           *server_proxy_;

protected:
	ServerTrackBase(const ServerTrackBase &);
	ServerTrackBase &operator=(const ServerTrackBase &);
};
