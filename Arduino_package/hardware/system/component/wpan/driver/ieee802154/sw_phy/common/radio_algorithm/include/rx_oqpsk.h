...
#define RX_DATA_SAMPLE_SIZE_HALF 7200 // 9600 * 3 / 2 / 2
...
struct RX_OQPSK {
	...
	uint32_t rx_post_pd_buf_idx;
	int32_t post_pd_gdma_cnt;
	int64_t cca_sample_re;
	int64_t cca_sample_im;


	uint32_t rx_pop_buf_idx;
	uint32_t rx_rssi_buf_idx;
	int64_t rx_rssi_dbm;
	int64_t rx_lqi;
	...
	uint32_t agc_break_cnt;
#if RX_INTERRUPT_DEBUG
	volatile uint32_t wl_rx_start_cnt;
	volatile uint32_t wl_rx_stop_cnt;
	volatile uint32_t r2r_cca_invalid_cnt;
	volatile uint32_t r2r_cnt;
	volatile uint32_t interrupt_state;
#endif
	void (*rx_start_callback)(uint8_t *frame);
} ;
