...
struct ifs_clm_para_info {
	...
	u16				mntr_time;	/*ms*/
	bool				ifs_clm_th_en[IFS_CLM_NUM];
...
#if RFDBG
	u32			start_time;	/*@monitor for the test duration*/
#endif
#ifdef NHM_SUPPORT
...
#endif
#ifdef EDCCA_CLM_SUPPORT
	u8		b_edcca_clm_manual_ctrl: 1;
	u8		b_edcca_clm_ongoing: 1;
#endif
#ifdef IFS_CLM_SUPPORT
	u8		ifs_clm_ongoing: 1;
#endif
#ifdef NHM_SUPPORT
	u8		b_nhm_manual_ctrl: 1;
	u8		b_nhm_ongoing: 1;
#ifdef NHM_DYM_PW_TH_SUPPORT
	u8		nhm_dym_pw_th_en: 1;
#endif
#endif
...
#ifdef FAHM_SUPPORT
void phydm_fahm_init(void);

void phydm_fahm_dbg(char input[][16], u32 *_used, char *output,
					u32 *_out_len);
#endif

#ifdef NHM_SUPPORT
void phydm_nhm_trigger(void);
bool phydm_nhm_get_result(void);
void phydm_nhm_init(void);

void phydm_nhm_dbg(char input[][16], u32 *_used, char *output,
				   u32 *_out_len);
#endif

#ifdef CLM_SUPPORT
void phydm_clm_c2h_report_handler(u8 *cmd_buf, u8 cmd_len);

void phydm_clm_dbg(char input[][16], u32 *_used, char *output,
				   u32 *_out_len);
#endif

u8 phydm_env_mntr_trigger(struct nhm_para_info *nhm_para,
						  struct clm_para_info *clm_para,
						  struct env_trig_rpt *rpt);

u8 phydm_env_mntr_result(struct env_mntr_rpt *rpt);

void phydm_env_mntr_dbg(char input[][16], u32 *_used,
						char *output, u32 *_out_len);

#ifdef IFS_CLM_SUPPORT
void phydm_ifs_clm_dbg(char input[][16], u32 *_used,
					   char *output, u32 *_out_len);
#endif
...
u8 phydm_enhance_mntr_result(struct enhance_mntr_rpt *rpt);

void phydm_enhance_mntr_dbg(char input[][16], u32 *_used,
							char *output, u32 *_out_len);

#ifdef EDCCA_CLM_SUPPORT
void phydm_edcca_clm_dbg(char input[][16], u32 *_used,
						 char *output, u32 *_out_len);
#endif

void phydm_env_mntr_result_watchdog(void);

void phydm_env_mntr_set_watchdog(void);

void phydm_env_monitor_init(void);
