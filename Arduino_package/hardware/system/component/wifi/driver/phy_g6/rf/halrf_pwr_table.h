...
#if !defined(RF_8730E_SUPPORT) && !defined(RF_8720E_SUPPORT) && !defined(RF_8721F_SUPPORT)
...
struct halrf_pwr_info {
	...
	u8 pwr_table_switch_efuse; /* 0: dis power by limit, en power by rate, 1: en both power by rate and limit, 2: disable both. */
	u8 pwr_tabel_switch_drv;   /* 0: dis power by limit, en power by rate, 1: en both power by rate and limit, 2: depend on efuse(flash). */

	u32 halrf_pwr_regulation[PW_LMT_MAX_BAND];
	u8 tx_shap_idx[PW_LMT_MAX_BAND][TX_SHAPE_MAX];
...
u8 halrf_get_regulation_info(u8 band);

s8 halrf_get_pwr_control(void);

bool halrf_pwr_is_minus(u32 reg_tmp);

s32 halrf_show_pwr_table(u32 reg_tmp);
