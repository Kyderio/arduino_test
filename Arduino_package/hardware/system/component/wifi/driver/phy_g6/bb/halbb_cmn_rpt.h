...
/*@--------------------------[Prptotype]-------------------------------------*/
void halbb_print_hist_2_buf_u8(u8 *val, u16 len, char *buf,
							   u16 buf_size);
void halbb_print_hist_2_buf(u16 *val, u16 len, char *buf,
							u16 buf_size);
u16 halbb_get_plurality_rx_rate_mu(void);
u16 halbb_get_plurality_rx_rate_su(void);
void halbb_basic_dbg_07_hist_su(void);
void halbb_show_rssi_and_rate_distribution_mu(void);
void halbb_show_rssi_and_rate_distribution_su(void);
void halbb_rx_pkt_cnt_rpt_reset(void);
#if ((defined HALBB_DBG_TRACE_SUPPORT) ||(defined CONFIG_PHYDM_CMD))
void halbb_rx_pkt_su_cnt_rpt(struct physts_rxd *desc, enum channel_width rx_bw);
void halbb_rx_pkt_su_rssi_statistic(void);
void halbb_rx_pkt_su_phy_hist(void);
void halbb_rx_pkt_cnt_rpt_beacon(struct physts_rxd *desc);
void halbb_rx_pkt_su_non_data_cnt_rpt(void);
void halbb_rx_pkt_mu_cnt_rpt(struct physts_rxd *desc, enum channel_width rx_bw);
void halbb_rx_pkt_mu_rssi_statistic(void);
void halbb_rx_pop_hist(void);
void halbb_physts_hist_init(void);
#endif

void halbb_cmn_rpt(struct physts_rxd *desc, u32 physts_bitmap);
void halbb_cmn_info_rpt_store_data(void);
void halbb_cmn_info_rpt_reset(void);
void halbb_cmn_rpt_init(void);
#endif
