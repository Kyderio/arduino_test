...
/*@--------------------------[Prptotype]-------------------------------------*/
bool halbb_ch_info_wait_from_physts(u32 dly, u32 dly_max,
									u16 bitmap, bool valid_rpt_only);
void halbb_ch_info_cfg_mu_buff_cr(bool en);
bool halbb_cfg_ch_info_cr(struct bb_ch_info_cr_cfg_info *cfg);
void halbb_ch_info_size_query(struct bb_ch_rpt_size_info *exp_rpt_size);
void halbb_ch_info_physts_en(bool en,
							 u16 bitmap, enum phl_phy_idx phy_idx);
void halbb_ch_info_status_en(bool en, enum phl_phy_idx phy_idx);
enum bb_ch_info_t halbb_ch_info_parsing(u8 *addr, u32 len,
										u8 *rpt_buf,
...
#ifdef CONFIG_CLINTWOOD
void halbb_ch_info_calc_pertone_snr(u8 snrvalue, u16 *addr, u32 len);
void halbb_ch_info_snr_parse(u8 *addr);
#endif
