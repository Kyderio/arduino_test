#ifndef _RL6845_REG_PAGE7_H_
#define _RL6845_REG_PAGE7_H_
...
#define BIT_HAS_AID0_1                            ((u32)0x00000001 << 15)          /*!<R 0  when set to 1 ,indicate has bc/mc frame */
#define BIT_HAS_MYBSSID1                          ((u32)0x00000001 << 14)          /*!<R 0  when set to 1 , indicate has my non-transmitted bss frame */
#define BIT_HAS_MYAID1                            ((u32)0x00000001 << 13)          /*!<R 0  when set to 1 , indicate has my unicast frame */
#define BIT_TIM_VALID1                            ((u32)0x00000001 << 12)          /*!<R 0  when set to 1 , indicate tim report valid */
 ...
 * @{
 **/
#define BIT_MASK_NDP_RX_STANDBY_TIMER             ((u32)0x000000FF << 24)          /*!<R/W 0x20  A timeout value for MAC to leave NDP_Rx_standby_state and go back to normal Rx mode. (unit: 2us) */
 ...
 * @{
 **/
#define BIT_WRITE_ENABLE                          ((u32)0x00000001 << 31)          /*!<W 0  Access flag. '0' for read. ‘1’ for write, 0x72C[15:0] must be sett before this bit is set */
...
#define BIT_GRANTALL_WLMASK                       ((u32)0x00000001 << 25)          /*!<R/W 0   */
 ...
 * @{
 **/
#define BIT_MASK_PTA_GNT_INFO                     ((u32)0x0000001F << 27)          /*!<R 0   */
#define BIT_PTA_GNT_INFO(x)                       (((u32)((x) & 0x0000001F) << 27))
#define BIT_GET_PTA_GNT_INFO(x)                   ((u32)(((x >> 27) & 0x0000001F)))
...
#define BIT_STATIS_BT_RSTB                        ((u32)0x00000001 << 19)          /*!<R/W 0  clear BT static */
...
/** @defgroup REG_BT_Isolation_Table_and_interrupt_Register_2
 * @brief
 * @{
 **/
#define BIT_BT_HID_ISR                            ((u32)0x00000001 << 31)          /*!<RW1C 0  When WLAN received the report packet including HID information, hardware will trigger an interrupt to notify firmware */
#define BIT_BT_QUERY_ISR                          ((u32)0x00000001 << 30)          /*!<RW1C 0  When WLAN received a query packet, hardware triggers an interrupt to notify firmware */
#define BIT_MAC_NULL_PKT_NOTIFY_ISR               ((u32)0x00000001 << 29)          /*!<RW1C 0  When MAC had sent NULL packet for BT and firmware also set nav_for_bt bit, hardware triggers an interrupt to notify firmware */
#define BIT_WLAN_RPT_ISR                          ((u32)0x00000001 << 28)          /*!<RW1C 0  When PTA sent the report packet successfully, hardware triggers an interrupt to notify firmware. */
#define BIT_BT_POWER_ISR                          ((u32)0x00000001 << 27)          /*!<RW1C 0  When WLAN received the packet including BT Power and check register “BT_STATUS_RPT”, if data contents aren't the same, hardware can trigger an interrupt to notify firmware */
#define BIT_BT_CHANNEL_ISR                        ((u32)0x00000001 << 26)          /*!<RW1C 0  When WLAN received the packet including BT channel number, hardware can trigger an interrupt to notify firmware. */
#define BIT_BT_SLOT_CHANGE_ISR                    ((u32)0x00000001 << 25)          /*!<RW1C 0  When WLAN received the packet including that BT had multi-slots, hardware can trigger an interrupt to notify firmware */
#define BIT_BT_PROFILE_ISR                        ((u32)0x00000001 << 24)          /*!<RW1C 0  When WLAN received the packet including that WLAN specific profile and check register “BT_STATUS_RPT”, if data contents aren't the same, hardware can trigger an interrupt to notify firmware. */
...
#define BIT_BT_ACT_FALLING_ISR                    ((u32)0x00000001 << 10)          /*!<RW1C 0  When BT_ACT signal is falling edge, it will trigger an interrupt. */
#define BIT_BT_ACT_RISING_ISR                     ((u32)0x00000001 << 9)          /*!<RW1C 0  When BT_ACT signal is raising edge, it will trigger an interrupt */
#define BIT_TDMA_TO_ISR                           ((u32)0x00000001 << 8)          /*!<RW1C 0  When BT_TIME expired, it will trigger an interrupt. */
 ...
 * @{
 **/
#define BIT_MASK_LTR_IDLE_LATENCY                 ((u32)0xFFFFFFFF << 0)          /*!<R/W 0x04444400  LTR idle latency. */
...
/** @defgroup REG_WMAC_STATE_CTRL
 * @brief
 * @{
 **/
#define BIT_WMAC_PSF_BCNTIMER_RST                 ((u32)0x00000001 << 10)          /*!<WA0 0  write pluse to reset fsm of wmac_psf_bcntimer */
#define BIT_WMAC_TRXPTCL_TXCTL_RST                ((u32)0x00000001 << 9)          /*!<WA0 0  write pluse to reset fsm of wmac_trxptcl_txctl */
#define BIT_WMAC_TRXPTCL_RESP_RST                 ((u32)0x00000001 << 8)          /*!<WA0 0  write pluse to reset fsm of wmac_ptrxptcl_resp */
#define BIT_WMAC_MACTX_CTL_PPDU_RST               ((u32)0x00000001 << 1)          /*!<WA0 0  write pluse to reset fsm of mactx_ctl */
#define BIT_WMAC_MACTX_CTL_RST                    ((u32)0x00000001 << 0)          /*!<WA0 0  write pluse to reset fsm of ppdu control in mactx_ctl */
/** @} */
 ...
 * @{
 **/
#define BIT_MASK_MASK_LA_MAC                      ((u32)0x00FFFFFF << 0)          /*!<R/W 0   */
#define BIT_MASK_LA_MAC(x)                        (((u32)((x) & 0x00FFFFFF) << 0))
#define BIT_GET_MASK_LA_MAC(x)                    ((u32)(((x >> 0) & 0x00FFFFFF)))
/** @} */
...
#define BIT_EN_TDE_DEBUG                          ((u32)0x00000001 << 4)          /*!<R/W 0  LA MODE function. enable txdma debug mode */
 ...
 * @{
 **/
#define BIT_MASK_R_WMAC_RXFIFO_FULL_TH_1          ((u32)0x000000FF << 24)          /*!<R/W 0x50  csi report mac pause bb almost full threshold */
 ...
 * @{
 **/
#define BIT_MASK_WMAC_RST_BB_CNT                  ((u32)0x00000007 << 29)          /*!<R/W 0x3  macclk cycle number of wmac_rst_bb signal */
#define BIT_WMAC_RST_BB_CNT(x)                    (((u32)((x) & 0x00000007) << 29))
#define BIT_GET_WMAC_RST_BB_CNT(x)                ((u32)(((x >> 29) & 0x00000007)))
#define BIT_MASK_WMAC_WAIT_PHY_REF                ((u32)0x0000003F << 23)          /*!<R/W 0x10  mactx hang handle, reset mactx if the phy_txen is set and wait the begin pulse of phy_txon timeout */
#define BIT_WMAC_WAIT_PHY_REF(x)                  (((u32)((x) & 0x0000003F) << 23))
#define BIT_GET_WMAC_WAIT_PHY_REF(x)              ((u32)(((x >> 23) & 0x0000003F)))
#define BIT_WMAC_CCK_LOCKED_CLK                   ((u32)0x00000001 << 22)          /*!<R/W 1  indicate that the transmit frequency and symbol clocks are derived from the same oscillator */
#define BIT_R_WMAC_MORE_DATA                      ((u32)0x00000001 << 21)          /*!<R/W 0  more data field in FC for ack/ba */
...
/** @defgroup REG_TXCMD_DBG_SEL
 * @brief
 * @{
 **/
#define BIT_MASK_TXCMD_DBG_SHIFT                  ((u32)0x00000007 << 8)          /*!<R/W 0   */
#define BIT_TXCMD_DBG_SHIFT(x)                    (((u32)((x) & 0x00000007) << 8))
#define BIT_GET_TXCMD_DBG_SHIFT(x)                ((u32)(((x >> 8) & 0x00000007)))
#define BIT_MASK_TXCMD_DBG_SEL                    ((u32)0x00000003 << 0)          /*!<R/W 0   */
 ...
 * @{
 **/
#define BIT_RXDEC_BM_DIS_CHKFC                    ((u32)0x00000001 << 23)          /*!<R/W 0  when set, disable check rx multicast management pkt protected frame in frame control when 0x680[3] and 0x7EC[22] are valid */
...
#define REG_BT_ISOLATION_TABLE_AND_INTERRUPT_REGISTER_20x078C
...
#define REG_RX_INT_CTRL                              0x07B8
#define REG_WMAC_STATE_CTRL                          0x07BC
#define REG_IQ_DUMP                                  0x07C0
#define REG_MASK_LA_MAC                              0x07C4
#define REG_MATCH_REF_MAC                            0x07C8
#define REG_WMAC_FTM_LA_CTL                          0x07CC
...
#define REG_TXCMD_DBG_SEL                            0x07E4
