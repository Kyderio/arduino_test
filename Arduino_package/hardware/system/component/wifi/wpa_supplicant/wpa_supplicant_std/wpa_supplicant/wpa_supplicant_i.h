...
#ifdef CONFIG_PASN_NAN
#define NAN_ENABLED BIT(0)
#define NAN_DONE BIT(1)
#define MAX_APPEND_NAN_IE 64
	int nan;
	u8 nan_role;
	u8 pairing_type;
	u8 cluster_id[ETH_ALEN];
	u8 peer_nmi[ETH_ALEN];
	u8 append_nan_ie[MAX_APPEND_NAN_IE];
	u8 append_nan_ie_len;
	u8 nik_caching;
	u8 own_pmkid[PMKID_LEN];
	u8 peer_pmkid[PMKID_LEN];
#endif /* CONFIG_PASN_NAN */
};
#endif /* CONFIG_PASN */
...
void wpas_pasn_nan_auth_start_config(struct wpa_supplicant *wpa_s,
                                     struct nan_pasn_trigger *info);
void wpas_pasn_nan_auth_rx(struct wpa_supplicant *wpa_s,
                      struct nan_pasn_rx *info);

void wpas_pasn_nan_upd_pmkid(struct wpa_supplicant *wpa_s,
                             struct nan_pasn_upd_pmkid *info);
