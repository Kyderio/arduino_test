...
#define BIT_NAND_RD   ((u32)0x00000001 << 12)           /*!< R/W 0x0  Set to enable SPIC when start NAND auto command flow in user read transactions. To start NAND auto command flow user read transaction, please do not set SSIENR.SPIC_EN. If NAND_RD is activated, SPIC will send Page read data command and Read data command to NAND Flash. Write: 0 - Not to activate NAND auto command flow user read 1 - Activate NAND auto command flow user read Read: 0 - NAND auto command flow user read is done 1 - NAND auto command flow user read is still in progress*/
...
#define MASK_ATOM_SIZE        ((u32)0x00000003 << 29)           /*!< R/W 0x0  Indicate the atomic size of read/write transactions 2'b00: 1 byte 2'b01: 2 byte 2'b10: 4 byte 2'b11: 3 byte The equation of atomic size: data_channel * data_rate e.g., if DQ16 and DDR, atomic size is (16*2/8) = 4 byte *Minimun atomic size is 1 byte *For exception of this equaltion, refer to PSRAM Format for devices with special requirements For TAG < 2.0.12, ATOM_SIZE is [7:6] and is set automatically by SPIC*/
...
/** @defgroup TPR0
  ...
  * @{
  */
#define MASK_CS_TCEM           ((u32)0x000000FF << 24)           /*!< R/W 0x0  Set to chop auto cmd when CSN low pulse width = (CS_TCEM*32) bus_clk. If set CS_TCEM = 0 will disable this function. ex. PSRAM tCEM = 4us, SPIC frequency = 100Mhz.  CS_TCEM = ((4000 ns/10 ns)/32) - BYTE_DELAY = 12 (c) - BYTE_DELAY If ((16*baud_rate)/data_channel) <=32. BYTE_DELAY = 0. If ((16*baud_rate)/data_channel) > 32. BYTE_DELAY = (((16*baud_rate)/data_channel) - 32)/32. ex. If baud_rate = 3, data_channel = 1.  BYTE_DELAY = (((16*3)/1) - 32)/32 = 16/32 = 0.5 Therefore, BYTE_DELAY should be 1 in this example. Note: Available only if configuration of PSRAM_EN is defined.*/
