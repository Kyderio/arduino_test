...
void *ota_update_malloc(u32 size);
...
int parse_http_response(unsigned char *response, u32 response_len, http_response_result_t *result);
...
u32 download_new_fw_from_server_http(u8 *first_buf, u32 firstbuf_len, int socket,
...
u32 download_new_fw_from_server_https(u8 *first_buf, u32 firstbuf_len, mbedtls_ssl_context *ssl,
									  update_ota_target_hdr *pOtaTgtHdr, u8 targetIdx, int index);
