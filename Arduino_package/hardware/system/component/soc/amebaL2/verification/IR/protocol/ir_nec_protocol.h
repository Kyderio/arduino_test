...
#define PULSE_HIGH				IR_BIT_TX_DATA_TYPE
/* No carrier waveform data type select */
#define PULSE_LOW				(0x0)
/* IR data mask */
#define IR_DATA_MSK				IR_MASK_TX_DATA_TIME
...
/**
  ...
  */
typedef struct {
	/* Unit of compensationFreq is Hz */
	uint32_t compFreq;
...
IR_RETURN_Type IR_NECEncode(uint8_t *data, IR_DataTypeDef *IR_DataStruct);
IR_RETURN_Type IR_NECEncodeComp(uint8_t *data, IR_DataTypeDef *IR_DataStruct, uint32_t fcomp);
IR_RETURN_Type IR_NECDecode(uint8_t *pdata, IR_DataTypeDef *IR_DataStruct, uint32_t fsample);
