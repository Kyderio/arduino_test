 ...
 */
#ifndef _AMEBA_IR_LEARN_H_
#define _AMEBA_IR_LEARN_H_
...
/** @defgroup IR_LEARN_Exported_Macros Ir Learn Exported Macros
    * @{
    */
// #define IR_IPCLK_100MHz             (100000000) /*<100MHz */
#define IR_IPCLK_100MHz             (40000000) /*<100MHz */
#define IR_FREQ_10MHZ               (10000000)  /*<10M*/
#define IR_SAMPLE_FREQ_NS           (1000000000/IR_FREQ_10MHZ)       /*1/IR_FREQ_10MHZ unit:ns */
#define IR_RX_FIFO_THR_LEVEL        (10)          /**<trigger interrupt if rx has IR_RX_FIFO_THR_LEVEL num data*/
#define LOOP_QUEUE_MAX_SIZE         (0x1000)
...
#define CALCULATE_FREQUENCY_FAST    TRUE
#if CALCULATE_FREQUENCY_FAST
#define CALCULATE_AVERAGE_CNT       200
#endif
/* Enable correcting frequency by software */
#define CORRECT_FREQ_BY_SOFTWARE    TRUE

/* IR learn time out is 10s */
#define IR_LEARN_TIME_OUT           (10*RTOS_TICK_RATE_HZ) /*10s*/

/*--------------Configure by user to confirm IR learn parameter --------*/
/* Filter threshold value. If 100< time interval< 40us, treat it as a part of a carrier time */
#define IR_LEARN_CARRIER_LOWEST_NS  (1000)
#define IR_LEARN_CARRIER_HIGHEST_NS (40000)
/* If low voltage time>50us,we treat it as a time which have no carrier */
#define IR_LEARN_NONCARRIER_TIME_NS (50000)
/* If low voltage time>5ms, ir learn stop */
#define IR_LEARN_STOP_TIME_NS       (5000000)
...
#define INVALID_KEY_ADDR_INDEX 0
/** End of IR_LEARN_Exported_Macros
    * @}
    */

#define IR_TIME2CNT(x)              ((x)/(IR_SAMPLE_FREQ_NS))
...
#if CALCULATE_FREQUENCY_FAST
	double               wave_frequency;    /**< record IR frequency*/
#else /* #if CALCULATE_FREQUENCY_FAST */
