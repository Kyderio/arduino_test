...
#define	FRAME_SIZE_1K	(4 + XM_BUFFER_SIZE)		/* X-modem structure (wr_offset + XM_BUFFER) */
#define	WAIT_FRAME_TIME		(4444444)				/* 1 sec, wait frame timeout, access reg cost 225ns */
#define	WAIT_CHAR_TIME		(WAIT_FRAME_TIME / 2)	/* 0.5 sec, wait char timeout */
#define	WAIT_HANDSHAKE_TIME	(WAIT_FRAME_TIME * 2)	/* 2 sec, handshake timeout */
#define	XMODEM_READ_MAXRETRANS		25
#define	FRAME_BUF_ADDR		(char*)(MSP_RAM_HP_NS - FRAME_SIZE_1K)
