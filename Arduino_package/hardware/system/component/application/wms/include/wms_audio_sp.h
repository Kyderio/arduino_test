...
#define ENABLE_SPEAKER_PIN	PB_31//PB_7 for testchip
#define ENABLE_LIGHT_PIN	PA_14
...
#define VOLUME_MIN 	0
#define VOLUME_MAX 	120   // 5V maximum 0x86(134), 12V maximum 0x96(150)
#define IE_SIZE 	18

#define SYNC_TIMER_IDX	3

typedef struct {
	int16_t pcm[PCM_PAGE_SIZE];
	u32 length;
	u32 seq;
	u32 rsvd[14]; 
}WMS_PCM_BLOCK;
...
struct wms_player_adapter {
	rtos_sema_t syncsema;		 /* play sync semaphore */ 
	rtos_queue_t pcm_queue;		/* pcm queue */ 
...
#define WS_DEBUG_LEVEL		WS_DEBUG
#if WS_DEBUG_ON
#define wms_printf(level, fmt, arg...)     \
do {\
	if (level <= WS_DEBUG_LEVEL) {\
		if (level <= WS_ERROR) {\
			printf("\n\rerror %s, " fmt "\n", __func__, ##arg);\
		} \
