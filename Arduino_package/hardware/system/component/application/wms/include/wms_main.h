...
#define SYNCUS_WMS			250000	/* us */
#define SYNCUS_LIPSYNC		30000	/* us */
...
#define AUDIO_CHANNEL_DATA		0x0001F000
...
#endif
#define PCM_POOL_SIZE		MAX_POOL_SIZE*2
#define TX_POOL_SIZE   		MAX_POOL_SIZE*2
#define RX_POOL_SIZE   		PCM_POOL_SIZE
#define RETRY_POOL_SIZE		MAX_POOL_SIZE*3
#define CACHE_POOL_SIZE		MAX_POOL_SIZE*2

extern struct wms_frame_info g_wms_frame_info;
...
enum SEMA_TYPE {
	SEMA_SOFTAP = 0,	/* softap init done semaphore */
	SEMA_PLAY, /* app request to play start */
	SEMA_WMS, /* wms mode */
	SEMA_MAX,
};

enum FLAG_TYPE {
	FLAG_RETRY = 0, /* frame rerty flag */
	FLAG_UNICAST, /* unicast flag */
	FLAG_SWITCH, /* udp address switch flag */
	FLAG_RECONNECT, /* wifi reconnect flag */
	FLAG_MAX,
};
...
struct node_list{
	struct node *next;
	rtos_sema_t nodesema;  /* node list protect sema */
};
...
typedef struct {
	...
	u8 segbuf[FRAME_SIZE];    /* frame segment buffer */
}rx_frame_seg;
...
struct wms_adapter {
	rtos_queue_t rx_queue;    /* aac frame queue */
	rtos_queue_t tx_queue;	 /* master udp tx frame queue */
	rtos_queue_t retry_queue;	 /* retry frame queue */
	_timer retry_timer;  /* child node frame retry timer */
	_timer heart_timer;  /* child node frame heartbeat timer */
	_timer node_chk_timer;   /* node state check timer */
	_timer bcn_timer;   /* replace beacon early */
	rtos_sema_t sema[SEMA_MAX];
...
void wms_conn_params_init(void);
void wms_stream_info_init(void);
...
void wms_create_lipsync_mode_thread(void);
int wms_nodelist_del(void);
void wms_slave_retry_timer_handler(void *param);
void wms_slave_heart_timer_handler(void *param);
int wms_softap_channel_get(void);
int wms_softap_channel_set(void);
void wms_create_app_thread(void);
...
void wms_cacheframe_release_page(void);
