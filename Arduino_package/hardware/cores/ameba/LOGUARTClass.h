...
class LOGUARTClass : public arduino::HardwareSerial {
    public:
        LOGUARTClass(int dwIrq, arduino::RingBuffer* pRx_buffer);
        void begin(const uint32_t dwBaudRate) { begin(dwBaudRate, SERIAL_8N1); }
        void begin(const uint32_t dwBaudRate, uint16_t serial_config_value);
        void end(void);
        int available(void);
        int peek(void);
        int read(void);
